...
向LCD指定起始位置写入一个字符串
*************************************/
void LCD12864_WriteStr(unsigned char x,unsigned char y,unsigned char *Str)
{
	if((y>3)||(x>7))
...
****************************************/
void LCD12864_PutPixel(unsigned char x,unsigned char y,unsigned char Color)
{
	unsigned char z,w;
	unsigned int Temp;
	if(x>=128||y>=64)
	...
	LCD12864_WriteCmd(y+0x80);        //行地址
	LCD12864_WriteCmd(x+z);     //列地址
    LCD12864_WriteData(Temp>>8);//先写入高8位，再写入低8位
    LCD12864_WriteData(Temp&0x00ff);
...
****************************************/
void LCD12864_HoriLine(unsigned char x,unsigned char y,unsigned char Length,unsigned char Color)
{
	unsigned char i;
	if(Length==0)
		return;
...
void LCD12864_Rectangle(unsigned char x0,unsigned char y0,unsigned char x1,unsigned char y1,unsigned char Color)
{
	...
	LCD12864_VertLine(x0,y0,y1-y0+1,Color);
	LCD12864_VertLine(x1,y0,y1-y0+1,Color);
	LCD12864_HoriLine(x0,y0,x1-x0+1,Color);
	LCD12864_HoriLine(x0,y1,x1-x0+1,Color);	
}
...
{
	LCD12864_PutPixel((x+yy),(y+xx),Color);//第 1 个 8 分圆
	LCD12864_PutPixel((x+xx),(y+yy),Color);//第 2 个 8 分圆
	LCD12864_PutPixel((x-xx),(y+yy),Color);//第 3 个 8 分圆
	LCD12864_PutPixel((x-yy),(y+xx),Color);//第 4 个 8 分圆
	LCD12864_PutPixel((x-yy),(y-xx),Color);//第 5 个 8 分圆
	LCD12864_PutPixel((x-xx),(y-yy),Color);//第 6 个 8 分圆
	LCD12864_PutPixel((x+xx),(y-yy),Color);//第 7 个 8 分圆
	LCD12864_PutPixel((x+yy),(y-xx),Color);//第 8 个 8 分圆
}

/******************************************
向LCD指定圆心坐标画一个半径为r的指定颜色的圆
*******************************************/
void LCD12864_Circle(unsigned char x,unsigned char y,unsigned char r,unsigned char Color)//中点法画圆
{//中点法画圆
	unsigned char xx,yy;
...
否则为全屏清除为颜色1(全屏填充显示)
******************************************/
void LCD12864_Clear(unsigned char Mode)
{
